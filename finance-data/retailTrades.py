...
config.read(
    os.path.abspath("C:/Users/menzi/Code/NQ_Tables/retail-activity/retailTrades.ini")
)
api_key = config.get("API", "api_key")

# Format: https://data.nasdaq.com/api/v3/datatables/{Table Code}/.{csv,json,xml}?api_key={api}
table_url = (
    f"https://data.nasdaq.com/api/v3/datatables/NDAQ/RTAT10.csv?&api_key={api_key}"
)
...
def send_to_discord_with_files(webhook_url, content, file_paths):
    ...
    for index, file_path in enumerate(file_paths):
        ...
        temp_file_path = os.path.join(
            output_folder, f"temp_{os.path.basename(file_path)}"
        )
    ...
    ):  # 204 is the expected successful response code from Discord for a webhook post
        print(
            f"Failed to send files to Discord. Response Code: {response.status_code}. Response: {response.text}"
        )
...
message = (
    f"Here's updated __**retail trader activity & sentiment**__ for {yesterday_date}!"
)

# Get all .csv files in the directory
all_files = os.listdir(output_folder)
csv_files = [
    os.path.join(output_folder, file) for file in all_files if file.endswith(".csv")
]
